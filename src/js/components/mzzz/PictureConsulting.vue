<template>
    <div>
        <div class="deal-content marginP remote-consultation-wrap content-bg-color">
            <AllHeader @getSelect="getSelect" :ywlx="ywlx" :activeName="activeName"></AllHeader>
            <div class="Header-add" v-show="shzt=='0'">
                <el-button class="btn" type="primary" @click="arranges">
                    <svg class="icon">
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template scope="scope">
                            <el-button
                                    size="small"
                                    @click="goTransferBill(scope.$index, scope.row,4)">转诊单
                            </el-button>
                            <el-button
                                    size="small"
                                    @click="handleEdit(scope.$index, scope.row)">{{scope.row.zzzt == '1' ? '撤销' : '重新预约'}}
                            </el-button>
                            <el-button
                                    size="small"
                                    @click="examineBill(scope.$index, scope.row)">{{scope.row.zzzt == '0' ? '编辑' : '查看'}}
                            </el-button>
                        </template>
                    </el-table-column>
                    <el-table-column
                        ...
                        <template scope="scope">
                            ...
                            <div v-show="scope.row.zzzt=='1'||scope.row.zzzt=='2'"
                                 :class="{active:scope.row.zzzt=='2',active1:scope.row.zzzt=='1'}">
                                {{scope.row.zzzt == '1' ? '成功' : '失败'}}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template scope="scope">
                            <el-button
                                    size="small"
                                    @click="goTransferBill(scope.$index, scope.row)">转诊单
                            </el-button>
                            <el-button
                                    size="small"
                                    @click="examineBill(scope.$index, scope.row)">{{scope.row.zzzt == '0' ? '编辑' : '查看'}}
                    ...
                    <el-table-column
                        ...
                        <template scope="scope">
                            <div v-show="scope.row.zzzt=='3'">取消</div>
                            <div :class="{active:scope.row.zzzt=='2',active1:scope.row.zzzt=='1'}"
                                 v-show="scope.row.zzzt=='1'||scope.row.zzzt=='2'">{{scope.row.zzzt == '1' ? '成功' : '失败'}}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
            ...
            <div class="MessageBox1" v-show="shows6">
                ...
                <p style="margin-top:10px;"><span>{{hosname}}</span><span
                        style="margin-left:20px;">{{deptname}}</span><span style="margin-left:20px;">{{patname}}</span>
                </p>
                <p style="margin-top:20px;"><span>{{qrtime}}</span><span style="margin-left:20px;">{{ampm}}</span><span style="margin-left:20px;">{{qrclock}}</span>
                </p>
                <el-button class="btn success1" type="text" style="padding:6px 8px;position:absolute;
right:100px;bottom:20px;" @click="SendAppoint">再想想
                </el-button>
                <el-button class="btn" type="primary" size="small" @click="arranges1"
                           style="position:absolute;bottom:20px;right:20px;"> 确认撤销
                </el-button>
            </div>
...
<script type="text/ecmascript-6">
    ...
    import {docCache, mzzzTabCache} from "../../lib/cache";
    ...
    export default{
        ...
        mounted(){
            ...
            let activeName = mzzzTabCache.get();
            if (activeName) {
        ...
        methods: {
            goTransferBill(index, row, actionName){
                mzzzTabCache.set(this.activeName)
