<template>
    <div>
        <div class="commonbox-box">
            ...
            <p style="font-weight: bold;font-size: 16px;text-align: center;margin-top:30px;" v-show="shows2">西湖区西溪街道社区卫生服务中心确认单</p>
            <div style="width:100%;height:100px;" v-show="shows2"></div>
            <p style="text-align: center;" v-show="shows1">西湖区西溪街道社区卫生服务中心确认单</p>
            <div class="commonbox-con">
                <div class="commonbox-news">
                    ...
                    <div v-show="zyzzList.ssmc"><span>手术名称:</span><span> {{zyzzList.qrssmc}}</span></div>
                    <div v-show="zyzzList.qrksmc"><span>预约科室:</span><span> {{zyzzList.ksmc}}</span></div>
                    <div v-show="zyzzList.jcmc"><span>检查项目:</span><span> {{zyzzList.qrjcmc}}</span></div>
                    <div v-show="zyzzList.jcdd"><span>检查地点:</span><span> {{zyzzList.jcdd}}</span></div>
                ...
                <div class="commonbox-news" style="margin-left:1%;">
                    <div v-show="zyzzList.flmc"><span>检查大类:</span><span> {{zyzzList.qrflmc}}</span></div>
                    <div v-show="!zyzzList.ssmc"><span>{{zyzzList.qrflmc?'就诊时间':'住院时间'}}:</span><span> {{zyzzList.yyrq}}</span><span style="margin-left:20px;">{{ampm}}</span></div>
                    <div v-show="zyzzList.ssmc"><span>手术时间:</span><span> {{zyzzList.yyrq}}</span><span style="margin-left:20px;">{{ampm}}</span></div>
...
<script>
    ...
    export default{
        ...
        mounted:function(){
            ...
            if(this.zyzzList.yylx=="0"){
                this.ampm="上午"
            }else if(this.zyzzList.yylx=="1"){
                this.ampm="下午"
            }
